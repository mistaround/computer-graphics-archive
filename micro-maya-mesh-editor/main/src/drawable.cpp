...
Drawable::Drawable(OpenGLContext* context)
    : count(-1), bufIdx(), bufPos(), bufNor(), bufCol(), bufJoints(), bufWeights(),
    idxBound(false), posBound(false), norBound(false), colBound(false), jointsBound(false), weightsBound(false),
      mp_context(context)
{}
...
{
    weightsBound = true;
    // Create a VBO on our GPU and store its handle in bufWeight
