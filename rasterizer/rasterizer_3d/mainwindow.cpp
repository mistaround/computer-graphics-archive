...
void MainWindow::keyPressEvent(QKeyEvent *e)
{
    ...
    case Qt::Key_W      : rasterizer.camera.TranslateForward(0.5f);   break;
    case Qt::Key_S      : rasterizer.camera.TranslateForward(-0.5f);  break;
    case Qt::Key_A      : rasterizer.camera.TranslateRight(-0.5f);    break;
    case Qt::Key_D      : rasterizer.camera.TranslateRight(0.5f);     break;
    case Qt::Key_Q      : rasterizer.camera.TranslateUp(-0.5f);       break;
    case Qt::Key_E      : rasterizer.camera.TranslateUp(0.5f);        break;
    case Qt::Key_Up     : rasterizer.camera.RotateRight(-5.f);        break;
    case Qt::Key_Down   : rasterizer.camera.RotateRight(5.f);         break;
    case Qt::Key_Left   : rasterizer.camera.RotateUp(5.f);            break;
    case Qt::Key_Right  : rasterizer.camera.RotateUp(-5.f);           break;
    case Qt::Key_Z      : rasterizer.camera.RotateForward(5.f);       break;
    case Qt::Key_X      : rasterizer.camera.RotateForward(-5.f);      break;
    }
    ...
    rasterizer(std::vector<Polygon>())
{
    ...
    connect(ui->AA, SIGNAL(valueChanged(int)), this, SLOT(slot_setAA(int)));
    connect(ui->LAMBER, SIGNAL(toggled(bool)), this, SLOT(on_checkBoxLambertian_toggled(bool)));
    connect(ui->TOON, SIGNAL(toggled(bool)), this, SLOT(on_checkBoxToon_toggled(bool)));
}
...
void MainWindow::slot_setAA(int c)
{
    rasterizer.antialiasing = c;
    rendered_image = rasterizer.RenderScene();
    DisplayQImage(rendered_image);
}

void MainWindow::on_checkBoxLambertian_toggled(bool checked)
{
    if (checked) ui->TOON->setChecked(false);
    rasterizer.shader = 1;
    if (!checked) rasterizer.shader = 0;
    rendered_image = rasterizer.RenderScene();
    DisplayQImage(rendered_image);
}

void MainWindow::on_checkBoxToon_toggled(bool checked)
{
    if(checked) ui->LAMBER->setChecked(false);
    rasterizer.shader = 2;
    if (!checked) rasterizer.shader = 0;
    rendered_image = rasterizer.RenderScene();
    DisplayQImage(rendered_image);
